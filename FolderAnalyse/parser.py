...
def parse(filename, exclusions=None, case_sensitive=False, sort=False):
    ...
    if sort:
        # Sort the dictionary by the values. Dictionarys are ordered in Python 3.6
        # and above but that is not the case for old versions where you must use
...
def sort_dict(frequency_dict, reverse=True):
    ...
    Note:
    ----
    Dictionarys are ordered in Python 3.6 and above but that is not the case 
    for old versions where you must use collections.OrderedDict.
    """
